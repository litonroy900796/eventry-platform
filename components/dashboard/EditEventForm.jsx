...
export default function EditEventForm({ event }) {
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
    <div className="space-y-1.5">
      ...
      <input type="date" name="date" defaultValue={formatDate(event.date)}
        className="w-full bg-[#27292F] border border-[#CCCCCC]/20 focus:border-indigo-500 focus:outline-none py-2.5 px-3 rounded-md text-white text-sm" />
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
    <div className="space-y-1.5">
      ...
      <input type="number" name="ticketPrice" defaultValue={event.ticketPrice ?? 0} min="0"
        className="w-full bg-[#27292F] border border-[#CCCCCC]/20 focus:border-indigo-500 focus:outline-none py-2.5 px-3 rounded-md text-white text-sm" />
    </div>
    <div className="space-y-1.5">
      <label className="text-[#9C9C9C] text-sm">Capacity <span className="text-[#555]">(optional)</span></label>
      <input type="number" name="capacity" defaultValue={event.capacity ?? ""} min="1" placeholder="Unlimited"
  ...
  <div className="space-y-1.5">
    ...
    <input type="url" name="website" defaultValue={event.website} placeholder="https://yourevent.com"
      className="w-full bg-[#27292F] border border-[#CCCCCC]/20 focus:border-indigo-500 focus:outline-none py-2.5 px-3 rounded-md text-white text-sm placeholder:text-[#555]" />
  </div>

  <div className="space-y-1.5">
    <label className="text-[#9C9C9C] text-sm">Image URL</label>
    <input type="text" name="imageUrl" defaultValue={event.imageUrl}
      className="w-full bg-[#27292F] border border-[#CCCCCC]/20 focus:border-indigo-500 focus:outline-none py-2.5 px-3 rounded-md text-white text-sm" />
  ...
  <div className="space-y-1.5">
    ...
    <textarea name="details" rows={4} defaultValue={event.details}
  ...
  {state?.success && (
    <p className="text-green-500 text-sm bg-green-500/10 border border-green-500/20 rounded-md px-3 py-2">
      {state.message}
    </p>
  )}
