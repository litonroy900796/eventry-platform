...
export async function createEvent(prevState, formData) {
  ...
  redirect(session.user.role === "admin" ? "/dashboard" : "/my-events");
}
// approve
...
export async function updateUserEvent(eventId, prevState, formData) {
  const session = await auth();
  if (!session) return { error: "Unauthorized" };
 ...
 const data = {
  ...
  try {
    ...
    if (event.createdBy.toString() !== session.user.id) return { error: "Unauthorized" };

    // approved হলে auto approved, rejected হলে pending
    const newStatus = event.status === "approved" ? "approved" : "pending";
